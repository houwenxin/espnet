...
preprocess_config=conf/no_preprocess.yaml  # conf/specaug.yaml
...
datadir=/root/work/espnet/egs/libriadapt/asr1/download/libriadapt_raw
...
accents="en-gb en-in en-us"
noise_types="clean"
devices="matrix nexus6 pseye respeaker shure usb"


train_set=en_us_clean_matrix/train
train_dev=en_us_clean_matrix/test
recog_set=""
    ...
    for accent in $accents; do
      for noise_type in $noise_types; do
        for device in $devices; do
          target_dir=$(echo ${accent} | tr - _)_${noise_type}_${device}
          if [ -f data/${target_dir}.complete ]; then
            echo "$0: data/$target_dir exists and appears to be complete."
          else
            # use underscore-separated names in data directories.
            local/data_prep.sh ${datadir} data/${target_dir} ${accent} ${noise_type} ${device}
            touch data/${target_dir}.complete
          fi
        done
      done
    done
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
    ...
    mkdir -p data/lang_char/$(echo ${train_set} | cut -d "/" -f1)
    echo "<unk> 1" > ${dict} # <unk> must be 1, 0 will be used for "blank" in CTC
    cut -f 2- -d" " data/${train_set}/text > data/lang_char/$(echo ${train_set} | cut -d "/" -f1)/input.txt
    spm_train --input=data/lang_char/$(echo ${train_set} | cut -d "/" -f1)/input.txt --vocab_size=${nbpe} --model_type=${bpemode} --model_prefix=${bpemodel} --input_sentence_size=100000000
    spm_encode --model=${bpemodel}.model --output_format=piece < data/lang_char/$(echo ${train_set} | cut -d "/" -f1)/input.txt | tr ' ' '\n' | sort | uniq | awk '{print $0 " " NR+1}' >> ${dict}
    wc -l ${dict}
